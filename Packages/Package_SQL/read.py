...
def show_all_students(students: list):
    ...
    print(f"{header[0]:^5}|{header[1]:^17}|{header[2]:^6}|{header[3]:^10}|{header[4]:^15}|{header[5]:^15}|{header[6]:^22}|{header[7]:^22}|{header[8]:^12}|{header[9]:^10}")
...
def show_student(student: tuple,title = True):
    ...
    message.append(f"{(student[1] if student[1] is not None else 'none') + ' ' + (student[2] if student[2] is not None else 'none'):^17}") # Name and Lastname
    ...
    message.append(f"{student[9] if student[9] is not None else 'none':^22}") # Mom's email
    message.append(f"{student[8] if student[8] is not None else 'none':^22}") # Dad's email
